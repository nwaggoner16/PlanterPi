...
config.read("/home/pi/PlanterPi/.config.ini")
...
class planter():
	...
	def water_logging(self, run_time, run_speed):
		cursor.execute('insert into Plant_logger.water_log (planter_id, run_time_sec, run_speed) values ({}, {}, {})'.format(self.planter_id, run_time, run_speed))
		mysql.commit()
...
class planter_by_id():
	...
	def get_planter_header_data(self, planter_id):	
		#Getting planter header info from planter table using planter_name
		temp_query = "SELECT planter_id, planter_name, cons_days_watered, times_watered, run_speed, run_time, target_moisture FROM Plant_logger.planter WHERE planter_id = {};".format(planter_id)
		cursor.execute(temp_query)
	...
	def get_planter_sensor_data(self):
		#Getting planter sensor data
		sensor_data_query = "SELECT planter_id, log_id, log_time, temperature, humidity, light, soil_temperature, soil_moisture FROM Plant_logger.sensor_data WHERE planter_id = {};".format(self.planter_id)
		cursor.execute(sensor_data_query)
